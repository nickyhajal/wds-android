 ...
 */
public class ChatRowAdapter extends ArrayAdapter<HashMap>{
    public ImageLoader mImageLoader;
    public Boolean isLoading = false;
    public ChatFragment mContext;

    public ChatRowAdapter(Context context, ArrayList<HashMap> items) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        mImageLoader.DisplayImage("http://avatar.wds.fm/"+item.get("av_id")+"?width=78", holder.avatar);
        holder.name.setText((String) item.get("with"));
        holder.content.setText((String)item.get("lastMsg"));
        Linkify.addLinks(holder.content, Linkify.WEB_URLS);
        holder.timestamp.setReferenceTime((long) item.get("lastStamp"));
